...
- when `-Dio.hawt.test.use.openshift=true` is specified, Hawtio Operator is used to deploy Hawtio Online in an OpenShift cluster (see **Hawtio Online (OpenShift Operator) E2E tests** section for details)
...
### Hawtio Online (OpenShift Operator) E2E tests

Hawtio Online tests deploy the **Hawtio Operator** to an OpenShift cluster and test the Hawtio console. The operator is deployed **cluster-wide** to the `openshift-operators` namespace.

#### Prerequisites
- Access to an OpenShift cluster (logged in via `oc login` or kubeconfig provided)
- Hawtio Operator catalog/index image (optional, defaults to Red Hat Operators catalog)

#### Basic OpenShift test execution

```console
mvn verify -f tests/hawtio-test-suite -Pe2e -Dio.hawt.test.use.openshift=true
```

This will:
1. Deploy the Hawtio Operator to `openshift-operators` namespace (cluster-wide)
2. Create a test namespace (randomly named `hawtio-tests-xxxxx`)
3. Deploy Hawtio CR in the test namespace
4. Run the test suite against the deployed Hawtio Online console
5. Clean up the test namespace after tests complete

#### Operator deployment modes

By default, the operator is deployed **clean** on each test run to ensure a fresh state. To control operator deployment behavior:

**Clean operator deployment (default):**
```console
mvn verify -f tests/hawtio-test-suite -Pe2e -Dio.hawt.test.use.openshift=true
```
This removes any existing operator resources (Subscription, CSV, CatalogSource) before deploying a fresh operator, ensuring:
- Clean state for consistent test results
- No orphaned resources from previous runs
- Proper operator deployment testing

**Reuse existing operator (faster):**
```console
mvn verify -f tests/hawtio-test-suite -Pe2e -Dio.hawt.test.use.openshift=true \
  -Dio.hawt.test.openshift.operator.clean=false
```
If an operator is already deployed from a previous run, it will be reused without redeployment. This is faster but may not catch operator deployment issues.

**Keep resources for debugging:**
```console
mvn verify -f tests/hawtio-test-suite -Pe2e -Dio.hawt.test.use.openshift=true \
  -Dio.hawt.test.openshift.namespace.delete=false \
  -Dio.hawt.test.openshift.operator.clean=false
```
This keeps both the test namespace and operator deployed after tests finish for manual inspection.

#### OpenShift configuration properties

All properties can be found in `io.hawt.tests.features.config.TestConfiguration` class:

- `-Dio.hawt.test.use.openshift=true` - Enable OpenShift deployment mode
- `-Dio.hawt.test.openshift.url=<url>` - OpenShift cluster URL (optional if logged in)
- `-Dio.hawt.test.openshift.username=<user>` - OpenShift username (default: `admin`)
- `-Dio.hawt.test.openshift.password=<pass>` - OpenShift password (default: `admin`)
- `-Dio.hawt.test.openshift.namespace=<name>` - Custom test namespace (optional, auto-generated by default)
- `-Dio.hawt.test.openshift.kubeconfig=<path>` - Path to kubeconfig file (optional)
- `-Dio.hawt.test.openshift.namespace.delete=true|false` - Delete test namespace after tests (default: `true`)
- `-Dio.hawt.test.openshift.operator.clean=true|false` - Clean operator deployment, removes existing operator before deploying (default: `true`)
- `-Dio.hawt.test.openshift.index.image=<image>` - Custom operator catalog/index image (optional)
- `-Dio.hawt.test.online.image.repository=<repo>` - Hawtio Online image repository override
- `-Dio.hawt.test.online.gateway.image.repository=<repo>` - Hawtio Gateway image repository override

#### Using custom operator catalog

To test with a custom operator catalog/index image:
```console
mvn verify -f tests/hawtio-test-suite -Pe2e -Dio.hawt.test.use.openshift=true \
  -Dio.hawt.test.openshift.index.image=quay.io/myorg/hawtio-operator-catalog:latest
```

#### Important notes

- The **Hawtio Operator is now cluster-wide only** (not namespace-scoped)
- Operator is deployed to `openshift-operators` namespace
- Hawtio CR (console) is deployed to the test namespace
- Operator resources are cleaned up only when `io.hawt.test.openshift.namespace.delete=true` (default)
- Reusing operator across runs significantly speeds up test execution
- Some tests are automatically skipped on OpenShift (e.g., JFR tests are tagged with `@notOnline`)
