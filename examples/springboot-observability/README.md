# Hawtio Spring Boot 4 Camel Observability Example

This sample application shows how to run Hawtio with Spring Boot 4 and Camel Observability.

## Camel Observability dependency

This example includes `camel-observability-services-starter` which provides health checks, metrics, and OpenTelemetry tracing for Camel routes:
...
The default Hawtio Spring Boot application exposes `hawtio` and `jolokia` endpoints. When adding Camel observability, include `prometheus` and `health` endpoints as well:

```properties
management.endpoints.web.exposure.include=hawtio,jolokia,prometheus,health
```

### Management base path

`camel-observability-services-starter` defaults the management base path to `/observe` instead of Spring Boot's `/actuator`, moving Hawtio to `/observe/hawtio/`. 

To keep standard Spring Boot URL patterns, configure:
...
- <http://localhost:10001/actuator/metrics> — Prometheus metrics
- <http://localhost:10001/actuator/health> — overall health status
- <http://localhost:10001/actuator/health/live> — liveness probe
- <http://localhost:10001/actuator/health/ready> — readiness probe
