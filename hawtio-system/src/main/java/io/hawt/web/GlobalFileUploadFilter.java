...
public class GlobalFileUploadFilter implements FileUploadFilter {
    ...
    public static synchronized boolean accept(byte[] fileContent, List<MagicNumberFileFilter> filters) {
        // By default all files are blocked unless configured via sys. prop
        ...
        try {
            while ((zipInputStream.getNextEntry()) != null) {
                ...
                if (outputStream.toByteArray().length > 0) {
                    ...
                    boolean configured = accept(outputStream.toByteArray(), fileFilter);
                    if (configured) {
                        continue; // valid file
                    }
    ...
    public static long getMaxFileSizeAllowed(List<MagicNumberFileFilter> filters) {
        if (filters.size() == 0) {
            return 100 * 1048576L; // Default file size 100MB
        }
