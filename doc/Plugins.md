...
**hawtio** is highly modular with [lots of plugins](http://hawt.io/faq/index.html#pluginList), so that it can discover exactly what services are inside a JVM and dyanmically update the console to provide an interface to them as things come and go.
...
For [all the plugins](http://hawt.io/faq/index.html#pluginList) we've done so far we've picked [AngularJS](http://angularjs.org/) as the UI framework, which has nice a two-way binding between the HTML markup and the JS data model along with modularisation, web directives and dependency injection.
