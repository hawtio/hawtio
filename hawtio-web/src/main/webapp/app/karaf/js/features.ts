    ...
    export function FeaturesController($scope, $location, workspace, jolokia) {
      ...
      */
        ...
        function featuresOfRepo(repository, features) {
            ...
            return features.findAll(function(feature) { return feature.repository === repository });
        }

        $scope.install = () => {
            $scope.selectedFeatures.forEach(function (feature) {
                installFeature(workspace,jolokia, feature.name, feature.version, function() {console.log("Installed!")}, function() {console.log("Failed to install!")});
            });
        }

        $scope.uninstall = () => {
            $scope.selectedFeatures.forEach(function (feature) {
                uninstallFeature(workspace, jolokia, feature.name, feature.version, function() {console.log("Uninstalled!")}, function() {console.log("Failed to uninstall!")});
            });
        }
